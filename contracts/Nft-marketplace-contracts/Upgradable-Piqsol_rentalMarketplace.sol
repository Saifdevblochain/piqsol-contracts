...
contract PiqsolRentalMarketplace is
    Initializable,
    ...
    UUPSUpgradeable
{
    ...
    modifier IsItemExists(uint256 id) {
        require(
            id < LentNFT_CollateralFeeForSale.length &&
                LentNFT_CollateralFeeForSale[id].id == id,
    ...
    function lendNFT_CollateralFee(
        ...
        uint256 startDateUNIX,
        uint256 endDateUNIX
    ) public HasTransferApproval(nftContract, tokenId) {
        require(
            IERC721Upgradeable(nftContract).ownerOf(tokenId) == msg.sender,
            "Piqsol: Not a nft owner"
        );
        require(
        ...
        require(
            pricePerDay > 0,
            "Piqsol:Rental price should be greater than 0"
        );
        require(
            startDateUNIX >= block.timestamp,
            "Piqsol:Start date cannot be in the past"
        );
        require(
            endDateUNIX >= startDateUNIX,
            "Piqsol:End date cannot be before the start date"
        );
    ...
    function rentNFT_CollateralFee(
        uint256 id,
        uint64 expires
    ) public payable IsItemExists(id) ItemIsForSale(id) {
        require(
            msg.sender != LentNFT_CollateralFeeForSale[id].owner,
            "Piqsol:Owner can't rent"
        );
        require(
            block.timestamp > LentNFT_CollateralFeeForSale[id].expires,
            "Piqsol:NFT already rented"
        );
        require(
            LentNFT_CollateralFeeForSale[id].expires <=
                LentNFT_CollateralFeeForSale[id].endDateUNIX,
            "Piqsol:Rental period exceeds max date rentable"
        );
        uint256 numDays = (expires - block.timestamp) / 60 / 60 / 24;
        uint256 rentalFee = LentNFT_CollateralFeeForSale[id].pricePerDay *
            numDays;
        uint256 totalfee = rentalFee +
            LentNFT_CollateralFeeForSale[id].collateralFee;
        require(
            msg.value >= totalfee,
            "Piqsol:Not enough ether to cover rental period"
        );
        ...
        emit RentedNFT_CollateralFee(
            id,
            IERC721Upgradeable(LentNFT_CollateralFeeForSale[id].nftContract)
                .ownerOf(LentNFT_CollateralFeeForSale[id].tokenId),
            msg.sender,
    ...
    function redeemFunds(uint256 id) public IsItemExists(id) {
        ...
        payable(LentNFT_CollateralFeeForSale[id].owner).transfer(
            LentNFT_CollateralFeeForSale[id].collateralReturnFee
        );
        ...
        IsItemExists(id)
    {
        ...
        uint256 num_days = (block.timestamp -
            LentNFT_CollateralFeeForSale[id].expires) /
            60 /
            60 /
            24;
        ...
        payable(LentNFT_CollateralFeeForSale[id].renter).transfer(
            LentNFT_CollateralFeeForSale[id].collateralReturnFee
        );
        LentActiveItems[LentNFT_CollateralFeeForSale[id].nftContract][
            LentNFT_CollateralFeeForSale[id].tokenId
     ...
     * unlist your rental
     */
    function cancelLending_CollateralFee(
        uint256 id
    ) external IsItemExists(id) ItemIsForSale(id) {
        require(
            msg.sender == LentNFT_CollateralFeeForSale[id].owner,
